...
> v1.2.1：写回前重读当前 title/details，若已被其他插件/他人修改则放弃对应字段写回（防 nfo 扫描竞态覆盖）
...
- **竞态保护**：写回前重读当前 title/details，若与本次读取的快照不一致（典型：nfoSceneParser 在扫描流程内改写标题，晚于本插件快照）则放弃对应字段写回，不覆盖他人刚写入的内容；重读失败/实体消失时保守跳过写回
