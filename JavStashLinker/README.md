...
> v1.5.1：交互细节优化 — ①「补全演员」**一次性**：点击执行完成后按钮置灰不可再点（无论成功/失败，含异常终止兜底），完成后仍显示剩余未命中数；指纹补全日志逐条记录**未命中原因（JAVStash 无该指纹的场景）+ 目标主名**；②手动搜索页演员**别名完整显示**，不再截断（自然换行，拉丁名按词、CJK 任意断行）；③「查看」改为**实心橘色按钮**（原文字链接），按钮层级更清晰；④「查看」与演员名跳转改为 **SPA 路由跳转**（`pushState` + 合成 `popstate`，走 Stash 的 react-router），不再触发整页刷新 — 面板状态与页面数据保持一致，浏览器回退/前进仍可用，Ctrl/⌘/Shift/Alt+点击或中键仍走浏览器默认行为（新标签页等）；⑤移除暂不使用的搜索接口保留代码；⑥「忽略」全面统一为**收窄行**（自动匹配/待审核/手动搜索页）：收窄为一行主名高度、主名转纯文本不可点击、隐藏证据描述/置信度徽章/「查看」按钮与行内「忽略」按钮，尾部「已忽略」徽章 + 「恢复」按钮，关闭面板后重置；自动匹配页忽略不再整组移除，「应用全部」照旧跳过已忽略项；⑦「别名搜索/模糊匹配」勾选框改为**小号圆角长条滑块开关**，位于「开始扫描」右侧、滑块在文字左侧（滑块靠右=开启，开启时滑轨显示主题色），点击滑块或文字均可切换，窄屏时自动换行到「开始扫描」下方；⑧数据大屏改为 **自动匹配 / 待审核 / 冲突 / 已忽略**（原 未匹配/手动选择）— 已忽略项不计入前三框（数量在「已忽略」框可见去向），页签计数按惯例保留总数不受忽略影响；冲突项现于自动匹配页正常显示冲突徽章（此前列表排除导致不可见，现仅「应用全部」排除），冲突框汇总冲突组数；⑨手动搜索页改用**紧凑提示区**（参照 performerMerge 别名修复页）：状态文案合并一行「未绑定演员 N 个 · M个单演员场景可补全（已忽略 K 个）」（筛选时首段变「匹配 X / Y 个」，补全中/完成后显示对应状态），「补全演员」更名**「补全单演员」**并与「开始扫描」左对齐（下方独立按钮行）
>
> v1.5.0：「开始扫描」重构为**双引擎串行** — 场景反推（引擎 A）后新增**名称搜索（引擎 B，借鉴 Javstash Autofill）**：对引擎 A 未覆盖、未绑定 JAVStash ID 的本地演员，用主名+全部别名逐词调 `searchPerformer` 交叉评级；「开始扫描」旁新增两个复选框 — **别名搜索**（默认开：主名+别名全量搜索、命中 high 即停）与 **模糊匹配**（默认关：开启后仅按名称相似度评级 ≥0.9 high / 0.7-0.9 medium，忽略 URL/StashDB/生日证据）；搜索匹配按证据评级（high/medium）分别进「自动匹配」「待审核」（与引擎 A 卡片同样式，不再单独标注「搜索」徽章）；新增**冲突护栏**：同一 JAVStash 演员被多个本地演员高可信命中时整组标记冲突徽章、「应用全部」跳过冲突项；手动搜索页新增**「补全演员」**：用本地单演员场景的指纹（phash/oshash）调 `findScenesBySceneFingerprints` 搜索 JAVStash，命中的唯一演员自动按标准应用路径补全（日志含成功/失败演员名单）；匹配卡片 **JAVStash 演员名**可点击（新标签页打开 JAVStash 演员页）、**查看** 链接在当前标签页打开本地演员 Stash 详情页，手动搜索页演员名/候选名同样可点击跳转；Python 批量任务同步接入引擎 B（固定 别名搜索√+模糊匹配✗ 最低风险配置，不受 UI 控制）
>
