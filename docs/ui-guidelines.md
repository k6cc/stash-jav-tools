...
| 中（动作按钮） | 分页/Tab 级主操作：查询、合并、清理、导出等 | padding 4×12，字号 12，定高 27px，inline-flex + line-height:1 |
| 小（行内/组按钮） | 结果卡片行内密集操作：应用、忽略、更多、▲ | padding 3×10，字号 11，定高 22px，inline-flex + line-height:1 |

> tagMerge（tgm）已落地中档（`<prefix>-btn-md`），新面板/新插件按上表取值；jsm/pdm 旧版未用中档，保持现状。三档高度约 31 / 27 / 22px，梯度递增。
...
- **按钮文字统一加轻投影**：三档按钮文字（白字在彩色实心底上）加 `text-shadow: 0 1px 1px rgba(0,0,0,0.3)`，声明在按钮基础类上**一次写、三档继承**，不在各档类重复；带图标的按钮与链接式按钮同样继承；透明框状态徽章不加（保持其半透明状态语义）
...
- **控件尺寸与同排按钮同高**：搜索框 / 下拉 select 与同行中档按钮对齐——定高 **27px**、字号 **12px**、`box-sizing: border-box`、垂直 padding 清零（`padding: 0 Xpx`，文字由控件默认垂直居中），水平 padding 另计（搜索框右侧需为内嵌清除按钮留位）。容器靠 `align-items: center` 垂直居中，**禁止用 margin 微调**；清除按钮保持 absolute 居中定位，框高变化不影响
