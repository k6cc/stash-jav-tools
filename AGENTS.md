...
### 列表筛选搜索框（含长列表的分页/面板应有）

**适用范围**：任何只能手动浏览编辑的分页（列表项 > 20 或需滚动才能看完），应提供搜索/筛选框——逐屏翻找不推荐但并非不允许（短列表、按序浏览有意义的场景可豁免）。JavStashLinker 手动搜索页、performerMerge 分组列表等均属此类。

**交互行为：**

- **即时过滤**：`oninput` 立即筛选，**不做防抖等待、不要求回车确认**；筛选在名称和别名（大小写不敏感的包含匹配）两个字段上执行
- **中文输入法（IME）兜底**：`oninput` 中 `e.isComposing` 为 true 时跳过（组字期间不重渲染，防打断候选框），同时监听 `compositionend` 做最终提交——部分输入法选字完成后不触发带最终值的 input 事件，只靠 oninput 会漏
- **焦点与光标保持**：重渲染后恢复输入框焦点和光标位置（渲染前记录 `selectionStart`，渲染后 `focus()` + `setSelectionRange`），否则每敲一个字符光标跳回末尾/丢焦
- **重渲染最小化**：列表本身用分块渲染（IntersectionObserver 按块追加），避免每次输入全量重建 DOM
