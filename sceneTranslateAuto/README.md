...
> v1.2.2：钩子入队延迟（默认 25s）等 nfo/javstashAutofill+ 先写入元数据（同场景多次入队覆盖合并）；写回前重读，已是目标语言则放弃、仍非目标语言则以当前值重译写回（防竞态且不误杀待翻译内容）
...
| 钩子入队延迟 / Hook enqueue delay (s) | STRING | `25` | 新场景创建/更新后延迟处理翻译，等 nfoSceneParser / javstashAutofill+ 先写入元数据；同场景多次入队覆盖合并，最后一次写入后静默 N 秒再处理；机械硬盘/多插件环境建议 30-60 |
...
| 入口 | 触发 | 行为 |
|---|---|---|
| 钩子 `Scene.Create.Post` / `Scene.Update.Post` | 场景创建/更新 | 语言预检（毫秒级，无网络）→ 需翻译则写入 pending 队列（带时间戳，同场景覆盖合并）并 spawn 单例后台 worker → worker 按「钩子入队延迟」到期后读当前标题翻译并写回，**顺带翻译关联图库（按「同步翻译关联图库」开关）**（hook 立即返回，不阻塞 Stash） |
...
- **竞态保护**：钩子入队延迟（默认 25s）让 nfoSceneParser / javstashAutofill+ 等插件先写完元数据，worker 读到的是"最后一次写入后"的快照；写回前再重读当前 title/details，若与本次快照不一致——**已是目标语言**则放弃对应字段写回（尊重 nfo 加工标题等新内容），**仍非目标语言**则以当前值重译写回（最多重译一次），保证"一切非目标语言最终翻译为目标语言"且不覆盖目标语言内容；重读失败/实体消失时保守跳过写回
...
| 文件 | 说明 |
|---|---|
| `sceneTranslateAuto.yml` | Stash 插件定义（hooks + 任务 + 5 项设置） |
