...
| [JavStashLinker](./JavStashLinker/) | 1.2.1 | Python + UI | 批量匹配 JAVStash 演员 ID，场景反推 + 番号确认 + 名称/别名匹配 + 手动搜索（多信号证据规则） |
...
| [tagMerge](./tagMerge/) | 2.1.0 | 纯 UI | 按可编辑映射库把相似名称的 tags（英文/日文/中文变体）合并为规范中文 tag，面板预览后一键合并，源名保留为别名；映射表可在面板内编辑并导出文件，冲突检测一键清理（无需 Python） |
