...
- **手动创建**：在影片编辑页面提供图标按钮，一键创建并实时反馈结果（成功/失败及原因）
- **批量补建**：在任务页面点击 "Create Gallery for Scene"，为所有未关联图库的场景批量创建
...
| **批量模式** | 任务页面 "Create Gallery for Scene" | 单任务串行 | 为所有未关联图库的场景批量创建 |
...
1. 进入影片编辑页面
2. 在"图库"标签旁点击蓝色文件夹加号图标按钮（悬停显示"创建图库"）
3. 按钮状态变化：创建中（旋转图标）→ 成功（绿色对勾）或 失败（红色叉，悬停显示失败原因）

失败原因提示（悬停按钮查看）：

| 提示 | 含义 |
|------|------|
| 未找到 extrafanart 文件夹 | 影片目录下没有 extrafanart 文件夹 |
| extrafanart 文件夹中没有图片 | 文件夹存在但内部无支持的图片文件 |
| 图片尚未入库（请先扫描） | extrafanart 图片还未被 Stash 扫描入库，手动模式不等待扫描 |

> 手动模式不依赖 nfoSceneParser，可在任何时候使用。手动模式直接读取当前数据库状态，不会等待图片入库（与自动模式不同），图片未入库时请先执行扫描。

### 批量模式

1. 进入 Stash 设置 → 任务页面，点击插件任务区的 "Create Gallery for Scene"
2. 插件查询所有**未关联图库**的场景，逐个处理：
   - 已关联图库的场景直接跳过（查询阶段过滤）
   - 目录下无 extrafanart 文件夹或文件夹内无图片的场景直接跳过（本地文件系统检查，不发请求）
   - extrafanart 图片尚未入库的场景跳过并计数（批量模式不等待入库，入库是扫描期间的事情）
   - 其余场景按后台创建逻辑创建图库并关联（含多碟追加、封面/fanart 导入）
3. 每处理 50 个场景输出一次进度统计，结束时输出汇总（created/updated、no_folder、no_images、not_indexed、failed 各计数）

> **串行不拥堵**：批量模式在单个插件任务内串行逐个处理，不派生子进程、不做入库轮询，同一时刻仅一个 GraphQL 请求，兼容 stash 单进程设置。多碟分集在批内自动共享图库（第一个分集创建，后续分集追加关联）。
...
### 1.9.5
- **编辑页按钮改为字段渲染即注入**：图库字段标签出现即注入按钮，不再等 300ms 防抖窗口 / 盲等定时器

### 1.9.1

- 更新插件 url 指向 Discourse 论坛帖子链接

### 1.8.0

- **新增批量模式**：任务页面点击 "Create Gallery for Scene"（无 scene_id 时）批量处理所有未关联图库的场景
  - 已关联图库的场景在查询阶段过滤；无 extrafanart 文件夹/无图片的场景本地检查跳过（不发请求）
  - 图片未入库的场景跳过并计数（不等待入库，入库是扫描期间的事情）
  - 单任务内串行处理，不派生子进程、不做入库轮询，同一时刻仅一个 GraphQL 请求
  - 每 50 个场景输出进度，结束输出汇总统计
  - 编辑页按钮（单场景）行为不变

### 1.7.0

- **手动按钮改为纯图标**：编辑页"创建图库"文字按钮改为蓝色文件夹加号图标（悬停显示文字说明），创建中显示旋转加载、成功显示绿色对勾、失败显示红色叉
- **手动按钮结果反馈**：按钮任务结束后自动判定真实结果并显示失败原因（未找到 extrafanart 文件夹 / 文件夹无图片 / 图片尚未入库等），不再只提示"任务已提交"
  - 前端轮询任务 job 状态，结束后解析插件日志判定结果（stash 对插件任务失败也标记 FINISHED，job 状态本身不可靠）
- **手动模式不再等待图片入库**：点击按钮时直接读取数据库状态，图片未入库立即返回失败提示（图片入库属于扫描阶段职责，自动模式行为不变，仍渐进轮询等待）
- **后台自动逻辑不受影响**：钩子模式 / 后台模式行为与 1.6.x 完全一致
