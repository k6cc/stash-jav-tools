...
> v1.2.4：新增插件设置「Force re-query」开关——该 box 已写过 id 的 tag 也重查、词缓存绕过（仅追加未写入实体），任务与钩子共用；填充逻辑与 tagMerge v2.7.2 同源
...
复用 tagMerge UI 版「填充ID」规则（v2.7.2 起同源），差别仅在无 UI 预览、后台串行执行：
...
- **全量重查**：插件设置「Force re-query」开关（默认关）开启时，该 box 已写过 id 的 tag 也处理、词缓存绕过重查（可捕捉 stash-box 新增实体/别名），仅追加未写入实体；默认关 = 该 box 已有任意 id 的 tag 直接跳过（has_id）
