...
> v1.1.1：修复与体验优化 — 合并第二层弹窗极窄屏溢出/删除按钮消失；拖拽排序重写为 Pointer Events + 幽灵行跟随（修复点击手柄页面卡死、触屏不可用）；拦截弹窗内拖动选中文本导致的原生拖拽会话卡死页面；列表行拖拽手柄/删除按钮内嵌输入框样式；注入按钮与删除按钮间距；新增幂等守卫防止脚本重执行堆积观察器。
